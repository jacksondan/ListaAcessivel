...
<body>
	...
	<div id="content2">
		...
		<%
			...
			Estabelecimento estabelciomentoDaSessao = (Estabelecimento) session
					.getAttribute("acessoEstabelecimento");
			Estabelecimento estabelecimento = fachada
					.pesquisarEstabelecimento(estabelciomentoDaSessao);
		...
		<form method="post"
			...
			<fieldset>
				...
				<label for="email">E-mail:</label> <input type="hidden"
					name="id_estabelecimento" id="email"
					value="<%=estabelecimento.getId_usuario()%>"> <input
					type="email" name="email" id="email"
					value="<%=estabelecimento.getEmail()%>" size=60> <label
					for="senha">Senha:</label><br> <input type="password"
					name="senha" id="senha" value="<%=estabelecimento.getSenha()%>"
					size=15>
			</fieldset>
			...
			<fieldset>
				...
				<label for="nome_fantasia">Nome Fantasia:</label> <input type="text"
					...
					value="<%=estabelecimento.getNome_fantasia()%>" size=60> <label
					for="nome_juridico">Nome Jurídico:</label> <input type="text"
					name="nome_juridico" id="nome_juridico"
					value="<%=estabelecimento.getNome_juridico()%>" size=60> <label
					for="cnpj">CPNPJ:</label><br> <input type="text" name="cnpj"
					id="cnpj" value="<%=estabelecimento.getCnpj()%>" size=12><br>

				Categoria:<br> <select name="categoria">
					<option selected="<%=estabelecimento.getCategoria()%>"
						value="<%=estabelecimento.getCategoria()%>"><%=estabelecimento.getCategoria()%></option>
			...
			<fieldset>
				...
				<p>
					<label for="lista de estados">Selecione o Estado:<br /> <select
						name="estado" id="listade estados">
							<option selected="<%=estabelecimento.getEstado()%>"
								value="<%=estabelecimento.getEstado()%>"><%=estabelecimento.getEstado()%></option>
				...
				<label for="cep">CEP:</label><br> <input type="text" name="cep"
					id="cep" value="<%=estabelecimento.getCep()%>" size=20><br>

				<label for="cidade">Cidade:</label><br> <input type="text"
					name="cidade" id="cidade" value="<%=estabelecimento.getCidade()%>"
					size=30><br> <label for="bairro">Bairro:</label><br>
				<input type="text" name="bairro" id="bairro"
					value="<%=estabelecimento.getBairro()%>" size=30><br>

				<label for="rua">Rua:</label><br> <input type="text" name="rua"
					id="rua" value="<%=estabelecimento.getRua()%>" size=60><br>

				<label for="numero">Número:</label><br> <input type="text"
					name="numero" id="numero" value="<%=estabelecimento.getNumero()%>"
					size=4><br> <label for="referencia">Referência:</label><br>
				<input type="text" name="referencia" id="referencia"
					value="<%=estabelecimento.getReferencia()%>" size=30><br>
